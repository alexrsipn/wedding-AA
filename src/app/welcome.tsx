    ...
    return isOpen ? (
        <>
            <section className={`w-screen h-screen z-50 fixed top-0 left-0 bg-black/50 flex items-center justify-center backdrop-blur-sm transition-opacity duration-500 ${isClosing ? "opacity-0" : "opacity-100"}`}>
                <div className={`bg-gray-50 dark:bg-slate-800 rounded-xl shadow-xl text-center flex flex-col gap-2 h-4/5 w-full lg:w-1/2 mx-4 -mt-20 lg:mt-0 transition-all duration-500 ${isClosing ? "scale-95 opacity-0" : "scale-100 opacity-100"}`} onClick={(e) => e.stopPropagation()}>
